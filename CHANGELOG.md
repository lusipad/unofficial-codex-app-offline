...
- 修复 CI `build-offline-package` 在商店新版本 `26.924.1866.0` 上重试全部失败的问题（issue #121）。上游 26.924 同时让四处静态补丁的锚点漂移，补丁器按设计失败关闭；CI 只报第一处，其余三处靠本地完整构建逐一暴露。均按 single-shape 规则重写为当前形态：
  1. **Chrome native pipe 前缀函数**：`browser-service.mjs` 把 `codex-browser-use` 管道前缀函数挪进 esbuild 惰性模块初始化器（`var qa,Pc=le(()=>{…qa=t=>…})`），声明与赋值不再相邻，原来要求 `var X=` 的正则失配（`Could not locate Chrome browser-client native pipe symbols…`）。现改为锚定赋值本身。
  2. **Chrome 直连 setup 判定**：平台分发器变成先处理 `BROWSER_USE_BACKEND_PATHS` 覆盖的 async 函数，末尾才 `return t.platform==="win32"?…`；改为锚定返回的三元分发。已确认 26.924 的 setup 流程里没有新的桥接守卫（桥接只剩共享工厂一处，已由传输补丁绕开），与 26.917 一样只需打标记。
  3. **Computer Use node_repl.js 动态工具调用桥**：handler 把选项解构从形参挪进函数体并去掉了 `queryClient`；同时 renderer 不再经总线发 `thread/start`，导致 app-server 请求总线名也找不到。handler 正则改为当前签名，总线改为锚定其定义（`AppServerManager RPC is not connected` → `forHost`），补丁器与验证器同步。**顺带修复一个潜伏问题**：替换代码此前硬编码了响应函数名 `qzn`/`Up`/`zl`，26.917 恰好对上；26.924 中正确的是 `Dwn`/`ae`/`TA`，而 `qzn`/`Up`/`zl` 仍存在但指向无关符号——只修签名会让桥接在运行时静默调用错误函数。现从 handler 自身紧随其后的原生响应调用中推导这三个名字，找不到即失败关闭。
  4. **侧栏 Activity 优先级视图**：门控函数的 return 多订阅了一个侧栏布局 atom（`return Y(rA),e&&(…)`），补丁器与验证器的已打/未打补丁正则同步改为当前形态。
- 删除补丁器中早已无引用的 node_repl 旧形态常量（`COMPUTER_USE_NODE_REPL_DYNAMIC_TOOL_CALL_RE`/`_CODE`/`_CURRENT_CODE`/`_COMPAT_MISSING_RE`），其中也硬编码了 `ln`/`G`/`X`/`Ge` 等压缩名，避免日后被误用。
- 修复 issue #120 及同类入口：`Codex.cmd`、`Codex Web.cmd`、`_internal\tools\Launch Codex Direct.cmd` 都不读取 `skill-installer.env`，而 `Setup Codex.cmd`（`bootstrap-codex-skills.ps1`）会读，导致按 README 设置 `CODEX_HOME` 后，安装写入了自定义目录，桌面端、Web 端却仍使用 `~/.codex`。三个启动器现共用构建脚本中唯一一份导入逻辑，按与 setup 相同的优先级导入 `CODEX_HOME`：已继承的环境变量优先，其次包根目录 `skill-installer.env`，再次 `_internal\skill-installer.env`，同一文件内第一个值生效，相对路径按包根目录解析；只导入 `CODEX_HOME`，不把 `GITHUB_TOKEN` 等技能安装器变量带进应用进程。安装版快捷方式与 `codex://` 协议均经过 `Codex.cmd`，一并生效。验证器要求三个启动器都包含该导入。
- 修复 Web 端自 26.917 起停在官方启动屏、26.924 仍无法进入主界面的问题（与 issue #112 同症，属 Gateway 层修复）。逐层定位到五处原因：
  1. **connect-app-host mock 未生效**：26.917 起官方把握手从独立的 `connect-app-host-*.js` chunk 并进 `app-shared-*.js`，而 Gateway 按文件名分派 mock，补丁函数从未被调用、连失配告警也不会触发。现按协议常量 `{type:\`connect-app-host\`` 分派。
  2. **补丁查询串写死导致旧补丁版本被缓存一年**：官方资源以 `immutable, max-age=1y` 缓存，查询串是唯一失效手段，却固定为 `codex-web-worked-for=1`；Gateway 修复后，访问过的浏览器仍执行旧的、未打补丁的 chunk（排查中曾因此得出假阴性）。现改为 `codex-web-patch=<补丁模块内容哈希>`，补丁一改自动失效。
  3. **新宿主 service `httpFetch` 缺失**：26.924 renderer 的 HTTP 请求改经宿主 `httpFetch` service（`fetch(id, req)`/`cancel(id)`），不再发 `fetch` 消息，Web 端报 `HTTP fetch service is unavailable.`。web-shell polyfill 新增适配器，把它转发到 Gateway 既有的 `fetch` 通道（复用 Statsig 兜底、ChatGPT 后端代理与离线降级），并按桌面主进程 `prepareFetchInit` 的约定为字符串 JSON body 补 `Content-Type: application/json`。
  4. **缺少 `codex-app-server-initialized`**：26.924 renderer 依赖这条桌面端消息中的 app-server 版本判定 gateway OAuth 就绪，收不到就一直 `loading`。Gateway 现于 app-server `initialize` 后按桌面契约广播（版本取自 `userAgent` 的 `<name>/<version>`），并与桌面端一样在 renderer 发 `ready` 时定向补发快照。
  5. **登录后 Statsig bootstrap 超时拖慢首屏**：26.924 新增 `POST /wham/statsig/bootstrap`（renderer 5 秒超时），离线代理只会超时。Gateway 与 `ab.chatgpt.com` initialize 一样本地返回默认特性。首屏进入时间由 20～30 秒降到约 11～14 秒。
- 删除已失效的 Web 个性化补丁（针对 26.901 前 RPC 不可用、用写死名 `Me=C(S,…)` 把 `codex-agents-md` 查询换成空 mock）：26.924 下它静默失配，而个性化页经 Gateway 已能正常加载自定义指令，继续匹配只会把真实数据替换为空。
- 验证：`node --test ./scripts/test/*.test.cjs ./web-gateway/gateway/test/*.test.cjs` 全绿（227 通过；`daily-launcher-codex-home.test.cjs` 以真实 `cmd.exe` 跑三个生成的启动器，`webHttpFetch`/`appServerInitialized`/`statsigBootstrap` 覆盖 Web 新契约）；`26.924.1866.0` 完整离线包构建（`-RequireInstaller`，补丁器 `all patches applied or already correct`）通过 `verify-offline-package.ps1`；Web 端在 26.924 上实测进入主界面、打开历史会话、进入设置 → 个性化均正常。

### English

- Fixed CI `build-offline-package` failing every retry on the Store's new `26.924.1866.0` (issue #121). Upstream 26.924 drifted four static-patch anchors at once and the patcher failed closed; CI only reports the first, the other three surfaced one by one in a full local build. Each is rewritten to the single current shape:
  1. **Chrome native pipe prefix helper**: `browser-service.mjs` moved the `codex-browser-use` pipe-prefix helper into an esbuild lazy module initializer (`var qa,Pc=le(()=>{…qa=t=>…})`), so its declaration and assignment are no longer adjacent and the `var X=` regex missed (`Could not locate Chrome browser-client native pipe symbols…`). It now anchors on the assignment.
  2. **Chrome direct setup detection**: the platform dispatcher became an async function that first honours `BROWSER_USE_BACKEND_PATHS` and only then `return t.platform==="win32"?…`; it now anchors on the returned ternary. Confirmed 26.924's setup flow has no new bridge guard (the bridge is only used by the shared factory, which the transport patch already bypasses), so as in 26.917 only the marker is needed.
  3. **Computer Use node_repl.js dynamic tool-call bridge**: the handler now destructures its options in the body and dropped `queryClient`; the renderer also stopped sending `thread/start` through the bus, so the app-server request bus name was not found either. The handler regex follows the new signature and the bus is anchored on its definition (`AppServerManager RPC is not connected` → `forHost`), in both patcher and verifier. **Also fixes a latent bug**: the replacement hard-coded the response helpers `qzn`/`Up`/`zl`, which happened to be right in 26.917; in 26.924 the right ones are `Dwn`/`ae`/`TA` while `qzn`/`Up`/`zl` still exist but name unrelated symbols — fixing only the signature would have made the bridge silently call the wrong functions at runtime. The three names are now derived from the handler's own native response call right after the claim, failing closed if absent.
  4. **Sidebar Activity priority view**: the gate function's return also subscribes to a sidebar layout atom (`return Y(rA),e&&(…)`); the patched/unpatched regexes in patcher and verifier follow the current shape.
- Removed the long-unreferenced legacy node_repl constants from the patcher (`COMPUTER_USE_NODE_REPL_DYNAMIC_TOOL_CALL_RE`/`_CODE`/`_CURRENT_CODE`/`_COMPAT_MISSING_RE`), which also hard-coded minified names such as `ln`/`G`/`X`/`Ge`, so nobody reuses them by mistake.
- Fixed issue #120 and its sibling entry points: `Codex.cmd`, `Codex Web.cmd` and `_internal\tools\Launch Codex Direct.cmd` did not read `skill-installer.env` while `Setup Codex.cmd` (`bootstrap-codex-skills.ps1`) does, so after setting `CODEX_HOME` there as the README describes, setup wrote to the custom home but the desktop app and Web gateway still used `~/.codex`. All three launchers now share the build script's single import routine with the same precedence as setup: an inherited environment variable wins, then the package-root `skill-installer.env`, then `_internal\skill-installer.env`; the first value in a file wins and relative paths resolve from the package root. Only `CODEX_HOME` is imported — skill-installer variables such as `GITHUB_TOKEN` stay out of the app processes. The installed shortcuts and the `codex://` protocol go through `Codex.cmd`, so they are covered too. The verifier requires the import in all three launchers.
- Fixed Web getting stuck on the official splash since 26.917 and still not reaching the main UI on 26.924 (same symptom as issue #112; fixed at the gateway layer). Five causes, found layer by layer:
  1. **connect-app-host mock never ran**: since 26.917 upstream folded the handshake from its own `connect-app-host-*.js` chunk into `app-shared-*.js`, while the gateway dispatched the mock by file name, so the patch never ran and its drift warning never fired. It now dispatches on the protocol constant `{type:\`connect-app-host\``.
  2. **Hard-coded patch query kept stale patches cached for a year**: official assets are served `immutable, max-age=1y` and the query string is the only eviction mechanism, yet it was fixed at `codex-web-worked-for=1`; after a gateway fix, browsers that had visited kept running the old unpatched chunk (this produced a false negative during the investigation). It is now `codex-web-patch=<hash of the patch module>`, so any patch change evicts automatically.
  3. **Missing `httpFetch` host service**: the 26.924 renderer sends HTTP through a host `httpFetch` service (`fetch(id, req)`/`cancel(id)`) instead of `fetch` messages, so Web failed with `HTTP fetch service is unavailable.`. The web-shell polyfill adds an adapter that forwards it to the gateway's existing `fetch` channel (reusing the Statsig fallback, ChatGPT backend proxy and offline degradation), and like the desktop main process's `prepareFetchInit` it adds `Content-Type: application/json` to string JSON bodies.
  4. **Missing `codex-app-server-initialized`**: the 26.924 renderer uses the app-server version from this desktop message to decide gateway OAuth readiness and stays `loading` without it. The gateway now broadcasts it per the desktop contract after app-server `initialize` (version parsed from `userAgent`'s `<name>/<version>`) and, like the desktop, replays the snapshot to a renderer when it sends `ready`.
  5. **Post-login Statsig bootstrap timeout slowed first paint**: 26.924 added `POST /wham/statsig/bootstrap` (5 s renderer timeout), which only times out when proxied offline. The gateway now answers it locally with the default features, like the `ab.chatgpt.com` initialize fallback. Time to the main UI dropped from 20–30 s to about 11–14 s.
- Removed the obsolete Web personalization patch (written for pre-26.901 RPC unavailability; it replaced the `codex-agents-md` query with an empty mock via hard-coded names `Me=C(S,…)`): on 26.924 it silently missed, and the personalization page now loads custom instructions through the gateway, so matching it again would only replace real data with an empty value.
- Verified: `node --test ./scripts/test/*.test.cjs ./web-gateway/gateway/test/*.test.cjs` all green (227 pass; `daily-launcher-codex-home.test.cjs` runs all three generated launchers under a real `cmd.exe`, and `webHttpFetch`/`appServerInitialized`/`statsigBootstrap` cover the new Web contracts); a full `26.924.1866.0` offline build (`-RequireInstaller`, patcher reports `all patches applied or already correct`) passes `verify-offline-package.ps1`; on 26.924 the Web UI reaches the main screen, opens an existing conversation, and loads Settings → Personalization.
...
- 按商店当前版本 `26.917.8451.0`（26.917 系列的后续构建）验证并发布一版：内置 codex CLI 为 `0.155.0-alpha.16.3`，恰好满足上游新增 `gpt-6-sol` / `gpt-6-luna` 的 `minimal_client_version = 0.155.0`；模型目录从 `rust-v0.155.0-alpha.16.3` tag 烘焙，在 astra 与三个 `gpt-5.6-*` 之外新增这两个 gpt-6 系列模型（上游 `visibility` 均为 `list`），合并后共 13 个模型。相对 `26.917.6896.0` 无补丁锚点漂移（补丁器全部应用成功，未触发失败关闭）。
- 验证：本地跑通 `26.917.8451.0` 完整离线包构建与 `verify-offline-package.ps1`（`direct-exe-survived-offline-launch`，app-server 与窗口均 ready，`app.asar` 补丁校验通过）。
- 修复 gateway 测试套件的两处过期期望与一处测试污染（CI 只跑 `scripts/test`，gateway 测试自 #113 起红灯一直未暴露；2026-09-13 起发布的版本均带此红灯）：① "source data contract covers direct exe asar patch surfaces" 硬编码的 marker 清单停留在补丁边界重划（#113，下线 9 个补丁）之前，现改为锁定 contract 全部 34 条在册记录（含 zero-change sentinel）——增删补丁记录必须同步评审这份清单；② "archived thread listing avoids workspace scans" 期望旧的三元形态 `useStateDbOnly:${archived}?!0:...`，现与 `verify-offline-package.ps1` 的判定对齐，同时接受 26.831 起上游固化的 `useStateDbOnly:!0`；③ 归档回环测试在模块加载前把 `CODEX_HOME` 指到临时目录：`desktopState` 每次读写都重读 `$CODEX_HOME/.codex-global-state.json`，用真实 `~/.codex` 时，正在运行的 Codex Desktop 在写读间隙重写该文件会让归档回读随机丢条目（全套件高负载下必现），且测试会把 `_test_archive_*` 垃圾写进真实用户状态（本次从用户状态文件清出 102 条）；`webCompat26908` 的 worktree 期望路径同步改为与 `GatewayCodexIpcPort.ts` 相同的 `CODEX_HOME` 回退链。
- 验证：`node --test ./scripts/test/*.test.cjs ./web-gateway/gateway/test/*.test.cjs` 连续 3 遍全绿（203 通过 0 失败）。
...
- Validated and released against the Store's current `26.917.8451.0` (a later build of the 26.917 train): the bundled codex CLI is `0.155.0-alpha.16.3`, which exactly meets the `minimal_client_version = 0.155.0` of the newly added upstream `gpt-6-sol` / `gpt-6-luna`; the model catalog is baked from the `rust-v0.155.0-alpha.16.3` tag and, on top of astra and the three `gpt-5.6-*` models, now includes these two gpt-6 models (both `visibility: list` upstream), 13 models in the merged catalog. No patch-anchor drift versus `26.917.6896.0` (all patches applied, fail-closed never triggered).
- Verified: full local build of `26.917.8451.0` passes `verify-offline-package.ps1` (`direct-exe-survived-offline-launch`, app-server and window both ready, `app.asar` patch checks pass).
- Fixed two stale expectations and one test-pollution case in the gateway test suite (CI only runs `scripts/test`, so the gateway suite had been red since #113 without anyone noticing; every release since 2026-09-13 shipped with it): (1) "source data contract covers direct exe asar patch surfaces" hard-coded a marker list from before the patch-boundary redesign (#113 dropped 9 patches) — it now locks the full contract inventory of 34 records (zero-change sentinels included), so adding/removing a patch record forces a review of this list; (2) "archived thread listing avoids workspace scans" expected the old ternary `useStateDbOnly:${archived}?!0:...` — it now matches the `verify-offline-package.ps1` judgment and also accepts the `useStateDbOnly:!0` form upstream has baked in since 26.831; (3) the archive round-trip test now points `CODEX_HOME` at a temp dir before module load: `desktopState` re-reads `$CODEX_HOME/.codex-global-state.json` on every get/set, and against the real `~/.codex` the running Codex Desktop rewrites that file between the test's write and read, so the archived entry randomly went missing (near-100% under full-suite load) — worse, the test wrote `_test_archive_*` junk into the real user state (102 entries cleaned out this time); `webCompat26908`'s expected worktree root now derives from the same `CODEX_HOME` fallback chain as `GatewayCodexIpcPort.ts`.
- Verified: `node --test ./scripts/test/*.test.cjs ./web-gateway/gateway/test/*.test.cjs` all green three consecutive runs (203 pass, 0 fail).
