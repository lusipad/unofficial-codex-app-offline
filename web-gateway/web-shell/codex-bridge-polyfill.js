(function () {
  ...
  const CODEX_WEB_SETTINGS_STORAGE_KEY = "codex_web_settings_v1";
  const CODEX_WEB_DEFAULT_SETTINGS = {
    mobileKeyboardOptimization: true,
    mobileSidebarAutoCollapse: true,
  };

  function codexWebSettings() {
    try {
      const parsed = JSON.parse(localStorage.getItem(CODEX_WEB_SETTINGS_STORAGE_KEY) || "{}");
      return {
        ...CODEX_WEB_DEFAULT_SETTINGS,
        ...(parsed && typeof parsed === "object" ? parsed : {}),
      };
    } catch {
      return { ...CODEX_WEB_DEFAULT_SETTINGS };
    }
  }

  function codexWebSettingEnabled(key) {
    return codexWebSettings()[key] !== false;
  }
  ...
  function markMobileComposerPromptSent(channel, payload) {
    if (!codexWebSettingEnabled("mobileKeyboardOptimization")) return;
  ...
  function shouldSuppressMobileComposerFocus(element) {
    if (!codexWebSettingEnabled("mobileKeyboardOptimization")) return false;
  ...
  function handleMobileSidebarConversationClick(event) {
    if (!codexWebSettingEnabled("mobileSidebarAutoCollapse")) return;
