...
/**
 * 创建一个最�?JSON-RPC client�?
 *
 * app-server 通过 WebSocket 使用 JSON-RPC；这里负�?request id、pending promise�?
 * 超时和连接断开时的统一失败处理�?
 */
function createJsonRpcClient(sendFn) {
  ...
  return {
    ...
    /** socket 断开�?dispose 时，把所有未完成请求一起失败掉�?*/
    failAll(error) {
...
/** �?CODEX_APP_SERVER_CMD 里推�?--listen endpoint，避免重复配�?URL�?*/
...
/** �?CODEX_APP_SERVER_CMD 里推�?WebSocket 地址，保留给�?health/兼容逻辑�?*/
...
/** 读取正数环境变量，非法值统一回退，避�?NaN 进入超时/TTL 计算�?*/
...
/**
 * 创建 gateway �?Codex app-server 的连接管理器�?
 *
...
function createCodexAppServerClient({ broadcast, logger, defaultCodexBinaryPath } = {}) {
  ...
  // responseCache 保存可缓�?RPC 的最近结果，减少远端设备加载时的一串阻塞请求�?
  const responseCache = new Map();
  // inflightRequests 用来合并同一�?cache key 的并发请求，避免首屏同时打爆 app-server�?
  ...
  const CACHEABLE_METHOD_TTLS_MS = new Map([
    ...
    ["thread/list", 2 * 60 * 1000],
    ["thread/loaded/list", 2 * 60 * 1000],
  ]);
  // stale TTL：超�?fresh 但还未过期时可以先返回旧数据，提升弱�?远端体验�?
  const CACHEABLE_METHOD_STALE_MS = new Map([
    ...
    ["thread/list", 10 * 60 * 1000],
    ["thread/loaded/list", 10 * 60 * 1000],
  ]);
  // 模型列表影响输入框右下角显示，优先预热�?
  ...
  /** 通过当前 transport �?app-server 写入 JSON-RPC 消息�?*/
  ...
  /** app-server notification 转成 gateway 广播，同时触发相关缓存失�?刷新�?*/
  ...
  function invalidateCacheForNotification(method) {
    ...
    if (method === "app/list/updated") {
      // app-server 已经把最新列表放�?notification payload 里；主动再拉 app/list
      // 会触发新�?app/list/updated，形成刷新风暴并挤占会话切换请求�?
  ...
  /** WebSocket 连接成功后释放等�?ensureConnection 的调用方�?*/
  ...
  async function handleAppServerMessage(raw, event = null) {
    try {
      ...
      // �?id 且有 method、没�?result/error 的消息，�?app-server �?renderer 处理的反向请求�?
  ...
  /** app-server 主动发来�?request id 统一转成字符�?key�?*/
  function serverRequestKey(id) {
    return String(id);
  }

  /** 记录 app-server 发给 renderer �?mcp-request，并广播�?web-shell 处理�?*/
  ...
  /** web-shell 返回 mcp-response 后，把响应转�?app-server�?*/
  ...
  /** 绑定 app-server WebSocket 生命周期，并�?JSON-RPC 消息分成响应、请求、通知三类�?*/
  ...
  /** 新版 Codex.app app-server 默认使用 stdio://，一行一�?JSON 消息�?*/
  ...
  /** unix://PATH 使用�?stdio 相同�?JSON-lines framing，只是底层换�?Unix domain socket�?*/
  ...
  /** 确保 app-server 已启动并连上；并发调用会复用同一�?connectionPromise�?*/
  ...
  async function request(method, params) {
    ...
    if (key) {
      ...
      if (cached && Date.now() < (cached.staleUntilMs || 0)) {
        // stale-while-revalidate：先返回旧数据保�?UI 快，再后台刷新�?
      ...
      // 同一�?key 的并发请求复用同一�?Promise，避免首屏多个组件重复请求�?
    ...
    if (method === "turn/start") {
      // turn/start 的结果用于兜底合�?turn/completed，避�?UI 一直显示进行中�?
      const threadId =
  ...
  /** 预热一组常�?app-server 请求；失败只记录日志，不阻塞 gateway 启动�?*/
